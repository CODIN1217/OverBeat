...
public class BaseCamera : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!handy.CompareWithBeforeValue(this.name, nameof(Update), nameof(playGM.curWorldInfoIndex), playGM.curWorldInfoIndex))
        {
            ...
            baseCamera.DOOrthoSize(stdSize * worldInfo.cameraInfo.size, worldInfo.cameraInfo.sizeTween.duration).SetEase(worldInfo.cameraInfo.sizeTween.ease);
            baseCamera.DOColor(handy.GetColor01(worldInfo.cameraInfo.BGColor), worldInfo.cameraInfo.BGColorTween.duration).SetEase(worldInfo.cameraInfo.BGColorTween.ease);
            transform.DORotate(Vector3.forward * handy.GetCorrectDegMaxIs0(worldInfo.cameraInfo.rotation), worldInfo.cameraInfo.rotationTween.duration).SetEase(worldInfo.cameraInfo.rotationTween.ease);
            transform.DOMove(worldInfo.cameraInfo.pos, worldInfo.cameraInfo.rotationTween.duration).SetEase(worldInfo.cameraInfo.rotationTween.ease);
